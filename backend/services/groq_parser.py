...
# Groq currently supports this model and JSON/structured output.
...
def parse_resume(raw_text: str) -> Dict:
    ...
    # First attempt failed — retry with stricter instruction.
    logger.warning(
        "Groq resume parse: first attempt returned invalid JSON, "
        "retrying..."
    )
    ...
    raise ValueError(
        "Groq returned an unparseable response after retry. "
        f"Raw response:\n{raw_response[:500]}"
    )
...
def parse_job_description(raw_text: str) -> Dict:
    ...
    logger.warning(
        "Groq JD parse: first attempt returned invalid JSON, "
        "retrying..."
    )
