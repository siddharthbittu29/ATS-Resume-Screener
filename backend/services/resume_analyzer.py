...
def analyze_full_resume(
    ...
    logger.info(
        f"Groq parsed summary: "
        f"{parsed_resume.get('professional_summary', '')[:100]!r}"
    )

    logger.info(
        f"Groq parsed skills count: "
        f"{len(parsed_resume.get('skills', []))}"
    )

    logger.info(
        f"Groq parsed experience count: "
        f"{len(parsed_resume.get('experience', []))}"
    )
    ...
    contact_info = {
        'email': parsed_resume.get(
            'email'
        ),
        'phone': parsed_resume.get(
            'phone'
        ),
        'linkedin': parsed_resume.get(
            'linkedin'
        ),
        'github': parsed_resume.get(
            'github'
        ),
        'portfolio': None,
    }
    ...
    # ========================================================
    #
    # SentenceTransformer has been removed.
    # validate_skills_with_projects now uses RapidFuzz.
    #
    ...
    # ========================================================
    # DEFAULT GRAMMAR / LOCATION RESULTS
    # ========================================================
...
def _generate_strengths(
    ...
    """
    Generate a list of things the resume does well,
    based on actual structured data.
    """
    ...
    if len(action_verbs) >= 5:
        ...
        strengths.append(
            f"Uses {len(action_verbs)} "
            f"strong action verbs in bullet points"
        )
    ...
    if len(validated) >= 3:
        ...
        strengths.append(
            f"{len(validated)} skills are "
            f"backed by project/experience evidence"
        )
