...
async def analyze_resume(
    request: Request,
    resume: UploadFile = File(..., description='Resume file â€” PDF or DOCX, max 5 MB'),
    ...
    try:
        ...
        raise HTTPException(
            status_code=422,
            detail='Could not read or parse the resume. Please check that the file is a valid PDF or DOCX.',
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail='Analysis failed due to an internal server error. Please try again.',
        )
...
async def health_check(request: Request):
    """Health check â€” confirms models are loaded and the API is ready."""
    return {
        'status': 'healthy',
...
async def get_history(user_id: str = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail='Could not load analysis history. Please try again.',
        )
...
async def delete_history_entry(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail='Could not delete the analysis. Please try again.',
        )
...
async def generate_history_pdf(
    ...
    try:
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": (
                    f"attachment; filename=ats_report_{analysis_id}.pdf"
                )
            },
        )
        ...
        raise HTTPException(
            status_code=500,
            detail='Failed to generate the history PDF. Please try again.',
        )